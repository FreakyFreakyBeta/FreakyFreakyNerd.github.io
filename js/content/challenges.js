function setupchallenges() {
  ...
  var c4bonus3 = new FunctionEffect(player.quarkstage.producers, EffectTypes.ProducerExponentialProduction, (amount) => Decimal.pow(new Decimal(Decimal.log(amount, 10)), .2).divide(250).add(1), (obj) => { return "Quark Production ^ 1+Log10(score)^.2/250 || Quark Production ^" + formatDecimalOverride(obj.value, 3) });
  player.challenges.push(new Challenge("c1x4", "[c4] Only 1", "Electrify and gain points, but you only can buy 1 Charger.", c4effects, [new Decimal("10"), c4bonus1, new Decimal("1e15"), c4bonus2, new Decimal("1e40"), c4bonus3], 1, () => { player.quarkstage.electrify.forceprestige(); }, null, () => new Decimal(Decimal.pow(player.quarkstage.quarks.gained, 1 / 2)), 1, [.1, 5, 5, 1], "(Quarks Gained)^.5"));
